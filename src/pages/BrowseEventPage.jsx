...
const EVENTS_ENDPOINT = "https://eventflow-backend-fwv4.onrender.com/api/events/get-events";
...
const getCategoryIcon = (category) => categoryIcons[String(category).toLowerCase()] || "bi bi-calendar-event";

const getEventTitle = (event) => event?.title || "Untitled Event";

const getEventId = (event, index = 0) => event?._id || `${getEventTitle(event)}-${index}`;
...
const BrowseEventPage = () => {
    ...
    const categories = useMemo(() => {
        const eventCategories = events.map((event) => getCategory(event)).filter(Boolean);
    ...
    const getLikeCount = (event, eventId) => {
        const currentLikes = Array.isArray(event?.likes) ? event.likes.length : 0;
                ...
                <div data-aos="fade-up" >
                    ...
                    {isLoading ? (
                        <div className="text-center py-5">
                            ...
                            <p className="mt-2 mb-0 text-secondary fw-semibold">Loading events...</p>
                        </div>
                    ) : error ? (
                        <div className="alert alert-warning mb-0 d-flex" role="alert">
                            {error}
                            <p>try again</p>
                        </div>
                        ...
                        <div className="text-center py-5 border rounded-4 bg-light">
                            ...
                            <p className="text-secondary mb-0">Try another category or check back later.</p>
                        </div>
                    ) : (
                            ...
                            {filteredEvents.map((event, index) => {
                                ...
                                return (
                                    <BrowseEvent
                                        ...
                                        date={formatEventDate(event?.startDateTime, event?.timeZone)}
                                        event={category}
                                        ...
                                        onAction={() => navigate("/dashboard/checkout")}
                                ...
                                <div className="modal-body p-4">
                                    <div className="d-flex align-items-start justify-content-between gap-3 mb-3">
                                        <div>
                                            <p className="card-text border gap-2 m-0 rounded-5 w-auto d-inline-flex px-2">
                                                <i className={getCategoryIcon(getCategory(selectedEvent))}></i>
                                                {getCategory(selectedEvent)}
                                            </p>
                                            <h4 className="mt-3 mb-2">{getEventTitle(selectedEvent)}</h4>
                                            <p className="text-secondary mb-0">{selectedEvent?.description || "No description has been added for this event."}</p>
                                        </div>
                                        <button
                                            type="button"
                                    ...
                                    <div className="row g-3 my-2">
                                        <div className="col-md-6">
                                            <div className="border rounded-4 p-3 h-100">
                                                <p className="text-secondary mb-1">Starts</p>
                                                <h6 className="mb-0">
                                                    <i className="bi bi-calendar-event me-2"></i>
                                                    {formatEventDate(selectedEvent?.startDateTime, selectedEvent?.timeZone)}
                                                </h6>
                                            </div>
                                        </div>
                                        <div className="col-md-6">
                                            <div className="border rounded-4 p-3 h-100">
                                                <p className="text-secondary mb-1">Ends</p>
                                                <h6 className="mb-0">
                                                    <i className="bi bi-clock me-2"></i>
                                                    {formatEventDate(selectedEvent?.endDateTime, selectedEvent?.timeZone)}
                                                </h6>
                                            </div>
                                        </div>
                                        <div className="col-md-8">
                                            <div className="border rounded-4 p-3 h-100">
                                                ...
                                                <h6 className="mb-0">
                                                    ...
                                                    {getFullAddress(selectedEvent)}
                                                </h6>
                                            </div>
                                        </div>
                                        <div className="col-md-4">
                                            <div className="border rounded-4 p-3 h-100">
                                                <p className="text-secondary mb-1">Ticket Price</p>
                                                <h6 style={{ color: "rgb(27,181,204)" }} className="mb-0">
                                        ...
                                        <div className="col-md-6">
                                            <div className="border rounded-4 p-3 h-100 d-flex align-items-center gap-3">
                                                {selectedEvent?.createdBy?.profilePic ? (
                                                    <img
                                                        src={selectedEvent.createdBy.profilePic}
                                                        alt={getHostName(selectedEvent)}
                                                        className="browse-event-host-img"
                                                    />
                                                ) : null}
                                                <div>
                                                    <p className="text-secondary mb-1">Hosted by</p>
                                                    <h6 className="mb-0">{getHostName(selectedEvent)}</h6>
                                                </div>
                                            </div>
                                        </div>
                                        <div className="col-md-6">
                                            <div className="border rounded-4 p-3 h-100">
                                                ...
                                                <h6 className="mb-0 text-capitalize">{selectedEvent?.status || "upcoming"}</h6>
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <div className="mt-3">
                                            ...
                                            <div className="d-flex flex-column gap-2">
                                                {selectedTicketTypes.map((ticket, index) => (
                                                    <div key={`${ticket?.name || "ticket"}-${index}`} className="border rounded-4 p-3 d-flex align-items-center justify-content-between gap-3">
                                                        <div>
                                                            <p className="fw-semibold mb-1">{ticket?.name || "General Admission"}</p>
                                                            <p className="text-secondary mb-0">{getAvailableTickets(ticket)}</p>
                                                        </div>
                                                        <p style={{ color: "rgb(223,127,7)" }} className="fw-bold mb-0">
                                                            {formatMoney(getTicketPriceValue(ticket) ?? 0)}
                                                        </p>
                                                    </div>
                                                ))}
                                            </div>
                                    ...
                                    <div className="d-flex align-items-center justify-content-end gap-3 mt-4">
                                        ...
                                        <button
                                            ...
                                            style={{ backgroundColor: "rgb(27,181,204)" }}
